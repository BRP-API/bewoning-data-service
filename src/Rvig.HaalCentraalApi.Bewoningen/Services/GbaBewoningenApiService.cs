...
public class GbaBewoningenApiService : BaseApiService, IGbaBewoningenApiService
{
	...
	private async Task<(GbaBewoningenQueryResponse bewoningenResponse, int afnemerCode)> GetBewoningenMedebewonersBase(string? identificatie, Func<string, bool, DateTime?, DateTime?, DateTime?, Task<(IEnumerable<GbaBewoning> bewoningen, int afnemerCode)>> getBewoningenFunc, bool checkAuthorization, DateTime? peildatum = null, DateTime? van = null, DateTime? tot = null)
	{
		...
		// Get bewoningen
		var bewoningenAfnemerCode = await getBewoningenFunc(identificatie, checkAuthorization, peildatum, van, tot);

		if (bewoningenAfnemerCode.bewoningen?.Any() != true)
		{
			return default;
		}

		var bewoningenResponse = new GbaBewoningenQueryResponse { Bewoningen = bewoningenAfnemerCode.bewoningen.ToList() };
		...
		return (bewoningenResponse, bewoningenAfnemerCode.afnemerCode);
	}
	...
	private async Task<(GbaBewoningenQueryResponse bewoningenResponse, List<long>? plIds)> GetBewoningen(BewoningMetPeildatum model)
	{
		...
		{
			throw new InvalidParamsException(new List<InvalidParams> { new InvalidParams { Code = "date", Name = "peildatum", Reason = "Waarde is geen geldige datum." } });
		}

		// Get bewoningen
		(GbaBewoningenQueryResponse bewoningenResponse, int afnemerCode) bewoningenResponseAfnemerCode = await GetBewoningenMedebewonersBase(model.adresseerbaarObjectIdentificatie, _getAndMapBewoningenService.GetBewoningen, _protocolleringAuthorizationOptions.Value.UseAuthorizationChecks, peildatumDateTime);

		var plIds = new List<long>();
		if (bewoningenResponseAfnemerCode.bewoningenResponse?.Bewoningen?.Any() == true)
		{
			// Filter response by date and fields
			bewoningenResponseAfnemerCode.bewoningenResponse.Bewoningen = FilterByPeildatumAndFields(peildatumDateTime, bewoningenResponseAfnemerCode.bewoningenResponse.Bewoningen!, $"{nameof(GbaBewoning.Periode)}.{nameof(Periode.DatumVan)}", $"{nameof(GbaBewoning.Periode)}.{nameof(Periode.DatumTot)}");

			var allBewonersForProtocollering = bewoningenResponseAfnemerCode.bewoningenResponse.Bewoningen?
						.Where(bewoning => bewoning?.BewonersPlIds?.Any() == true)
						.SelectMany(bewoning => bewoning.BewonersPlIds!)
				.Concat(bewoningenResponseAfnemerCode.bewoningenResponse.Bewoningen
			...
			{
				await LogProtocolleringInDb(bewoningenResponseAfnemerCode.afnemerCode, plIds,
		...
		return (bewoningenResponseAfnemerCode.bewoningenResponse ?? new GbaBewoningenQueryResponse { Bewoningen = new List<GbaBewoning>() }, plIds);
	}
	...
	private async Task<(GbaBewoningenQueryResponse bewoningenResponse, List<long>? plIds)> GetBewoningen(BewoningMetPeriode model)
	{
		...
		{
			throw new InvalidParamsException(new List<InvalidParams> { new InvalidParams { Code = "date", Name = "datumVan", Reason = "Waarde is geen geldige datum." } });
		}
		else if (!DateTime.TryParse(model.datumTot, out datumTotDateTime))
		{
			throw new InvalidParamsException(new List<InvalidParams> { new InvalidParams { Code = "date", Name = "datumTot", Reason = "Waarde is geen geldige datum." } });
		}
		else if (datumVanDateTime >= datumTotDateTime)
		{
			throw new InvalidParamsException(new List<InvalidParams> { new InvalidParams { Code = "date", Name = "datumTot", Reason = "datumTot moet na datumVan liggen." } });
		}

		// Get bewoningen
		var bewoningenResponseAfnemerCode = await GetBewoningenMedebewonersBase(model.adresseerbaarObjectIdentificatie, _getAndMapBewoningenService.GetBewoningen, _protocolleringAuthorizationOptions.Value.UseAuthorizationChecks, null, datumVanDateTime, datumTotDateTime);
		...
		{
			// Filter response by date and fields
			bewoningenResponseAfnemerCode.bewoningenResponse.Bewoningen = FilterByDatumVanDatumTot(datumVanDateTime, datumTotDateTime, bewoningenResponseAfnemerCode.bewoningenResponse.Bewoningen!, $"{nameof(GbaBewoning.Periode)}.{nameof(Periode.DatumVan)}", $"{nameof(GbaBewoning.Periode)}.{nameof(Periode.DatumTot)}");
			...
			if (_protocolleringAuthorizationOptions.Value.UseProtocollering)
			{
				await LogProtocolleringInDb(bewoningenResponseAfnemerCode.afnemerCode, allBewonersForProtocollering?.Select(x => x.plId).Distinct().ToList(),
										new List<string> { "081030", "081180", "081320" },
										new List<string> { "010120" }
									);
			}
