...
public class RvigBewoningenRepo : RvigRepoPostgresBase<bewoning_bewoner>, IRvigBewoningenRepo
{
	...
	public RvigBewoningenRepo(IOptions<DatabaseOptions> databaseOptions, IOptions<HaalcentraalApiOptions> haalcentraalApiOptions, ILoggingHelper loggingHelper) : base(databaseOptions, haalcentraalApiOptions, loggingHelper)
	{
	...
	private async Task<DbBewoningWrapper> GetBewoningByAdresseerbaarObjectIdentificatie(string adresseerbaarObjectIdentificatie)
	{
		...
		return new DbBewoningWrapper
		{
			Bewoners = bewoningBewoners.Where(bewoningBewoner => !string.IsNullOrWhiteSpace(bewoningBewoner.vb_adres_functie) && !bewoningBewoner.vb_adres_functie.ToUpper().Equals("B")),
			verblijf_plaats_ident_code = adresseerbaarObjectIdentificatie,
		};
	}
	...
	public async Task<IEnumerable<(BewoningOnBsnWrapper bewoningOnBsnWrapper, IEnumerable<(lo3_adres, lo3_pl_verblijfplaats)> adressenVerblijfplaatsen, IEnumerable<lo3_pl_persoon> medebewoners)>> GetMedebewoners(string bsn)
	{
		...
		foreach (BewoningOnBsnWrapper bewoning in bewoningOnBsnWrappers)
		{
			...
			//.Where(medebewoner => !medebewoner.burger_service_nr.Equals(long.Parse(bsn)));d
			// Add person in question yes or no?
	...
	//private async Task<IEnumerable<DbBewoningWrapper>> ExecuteBewoningQuery(string query, DynamicParameters dynamicParameters)
	//{
	//	var types = new[] { typeof(lo3_adres), typeof(lo3_pl_verblijfplaats) };

	//	const string splitOn = $"{nameof(lo3_adres.adres_id)},{nameof(lo3_pl_verblijfplaats.pl_id)}";
	//	var bewoningen = await DapperQueryAsync(query, types, obj => MappingBewoningFunction(obj), dynamicParameters, splitOn: splitOn);

	//	return new List<DbBewoningWrapper> { new DbBewoningWrapper { Adressen = bewoningen.SelectMany(bewoning => bewoning.Adressen), Verblijfplaatsen = bewoningen.SelectMany(bewoning => bewoning.Verblijfplaatsen) } };
	//}
	...
	//private static DbBewoningWrapper MappingBewoningFunction(object[] obj)
	//{
	...
	//	var bewoningWrapper = new DbBewoningWrapper();
	//	(bewoningWrapper.Adressen as List<lo3_adres>)?.Add(adres);
	//	(bewoningWrapper.Verblijfplaatsen as List<lo3_pl_verblijfplaats>)?.Add(verblijfplaats);
	//	return bewoningWrapper;
	//}
