...
namespace Rvig.Data.Bewoningen.Mappers.Helpers;
public static class GbaMappingHelper
{
    ...
    public static DateTime? GetNullableDateTimeFromIncompleteDateString(string? objStartDate)
    {
        if (string.IsNullOrWhiteSpace(objStartDate))
        {
            return null;
        }

        return GetDateTimeFromIncompleteDateString(objStartDate);
    }

    public static DateTime GetDateTimeFromIncompleteDateString(string? objStartDate)
    {
        var objStartDateOnvolledig = new DatumOnvolledig(objStartDate);
        if (objStartDateOnvolledig.IsCompleteOnvolledig())
        {
            return DateTime.MinValue;
        }
        else if (objStartDateOnvolledig.IsOnvolledig() && objStartDateOnvolledig.Jaar.HasValue && !objStartDateOnvolledig.Maand.HasValue)
        {
            return DateTime.ParseExact($"{objStartDateOnvolledig.Jaar}", "yyyy", CultureInfo.InvariantCulture);
        }
        else if (objStartDateOnvolledig.IsOnvolledig() && objStartDateOnvolledig.Jaar.HasValue && objStartDateOnvolledig.Maand.HasValue)
        {
            return DateTime.ParseExact($"{objStartDateOnvolledig.Jaar}{objStartDateOnvolledig.Maand.Value.ToString().PadLeft(2, '0')}", "yyyyMM", CultureInfo.InvariantCulture);
        }
        else
        {
            return DateTime.ParseExact(objStartDate!, "yyyyMMdd", CultureInfo.InvariantCulture);
        }
    }
    ...
    private static bool CurrentDateOnzekerheidsPeriodeOverlapVanTotPeriode(DatumOnvolledig current, DateTime? periodeVan, DateTime? periodeTot, bool? previousDateOverlapsWithCurrent = false, DatumOnvolledig? previous = null, bool? nextDateOverlapsWithCurrent = false, DatumOnvolledig? next = null)
    {
        ...
        if (nextDateOverlapsWithCurrent == true && next?.IsOnvolledig() == false)
        {
            endOnzekerheidsPeriodeDateTime = DateTime.Parse(next.Datum!);
        }
        else if (nextDateOverlapsWithCurrent == true && next?.IsOnvolledig() == true)
        {
        ...
        if (previousDateOverlapsWithCurrent == false && previous?.IsOnvolledig() == false)
        {
            startOnzekerheidsPeriodeDateTime = DateTime.Parse(previous.Datum!);
        }
        else if (previousDateOverlapsWithCurrent == false && previous?.IsOnvolledig() == true)
        {
    ...
    {
        DateTime? startOnzekerheidsPeriodeDateTime;
        DateTime? endOnzekerheidsPeriodeDateTime;
