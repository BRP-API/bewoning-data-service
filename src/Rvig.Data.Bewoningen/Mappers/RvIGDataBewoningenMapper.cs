...
public class RvIGDataBewoningenMapper : RvIGDataMapperBase, IRvIGDataBewoningenMapper
{
	...
	public GbaBewoning? MapBewoning(List<(bewoning_bewoner dbBewoner, long plId)> dbBewonersPlIds, List<(bewoning_bewoner dbBewoner, long plId)> dbMogelijkeBewonersPlIds, string? verblijf_plaats_ident_code, DateTime? peildatum = null, DateTime? van = null, DateTime? tot = null)
	{
		...
		List<(GbaBewoner gbaBewoner, long plId)>? bewoners = opgeschorteBewoners
			.Select(dbBewonerPlId => (new GbaBewoner
			{
		...
		List<(GbaBewoner gbaBewoner, long plId)>? mogelijkeBewoners = opgeschorteMogelijkeBewoners
			.Select(dbMogelijkeBewonerPlId => (new GbaBewoner
			{
	...
	private static string? AddHyphensToDateString(string? dateString)
	{
		...
		return $"{int.Parse(dateString.Substring(0, 4)).ToString().PadLeft(4, '0')}-{int.Parse(dateString.Substring(4, 2)).ToString().PadLeft(2, '0')}-{int.Parse(dateString.Substring(6, 2)).ToString().PadLeft(2, '0')}";
	}
	...
	private bool IsBewonerOpgeschort(string? opschortingReden, int? opschortingDatum, DateTime? peildatum = null, DateTime? van = null, DateTime? tot = null)
	{
	...
	{
        //throw new CustomNotImplementedException("Reimplement medebewoner mapping.");
        var gbaBewoningen = new List<GbaBewoning>();
		foreach (var (bewoningOnBsnWrapper, adressenVerblijfplaatsen, medebewoners) in bewoningen)
		{
			var gbaBewoning = new GbaBewoning
			{
				AdresseerbaarObjectIdentificatie = bewoningOnBsnWrapper.verblijf_plaats_ident_code,
				//Adressen = adressenVerblijfplaatsen
				//	.Select(adresVerblijfplaats =>
				//	{
				//		var gemeenteVanInschrijving = GetGemeenteVanInschrijving(adresVerblijfplaats.verblijfplaats);
				//		return (MapVerblijfplaats<GbaVerblijfplaats>(adresVerblijfplaats.verblijfplaats, adresVerblijfplaats.adres, gemeenteVanInschrijving), gemeenteVanInschrijving);
				//	})
				//	?.Select(verblijfplaatsGemeenteVanInschrijving => new GbaVerblijfadres
				//	{
				//		AanduidingBijHuisnummer = verblijfplaatsGemeenteVanInschrijving.Item1?.AanduidingBijHuisnummer,
				//		GemeenteVanInschrijving = verblijfplaatsGemeenteVanInschrijving.gemeenteVanInschrijving,
				//		Huisletter = verblijfplaatsGemeenteVanInschrijving.Item1?.Huisletter,
				//		Huisnummer = verblijfplaatsGemeenteVanInschrijving.Item1?.Huisnummer,
				//		Huisnummertoevoeging = verblijfplaatsGemeenteVanInschrijving.Item1?.Huisnummertoevoeging,
				//		Land = verblijfplaatsGemeenteVanInschrijving.Item1?.Land,
				//		Locatiebeschrijving = verblijfplaatsGemeenteVanInschrijving.Item1?.Locatiebeschrijving,
				//		Postcode = verblijfplaatsGemeenteVanInschrijving.Item1?.Postcode,
				//		Regel1 = verblijfplaatsGemeenteVanInschrijving.Item1?.Regel1,
				//		Regel2 = verblijfplaatsGemeenteVanInschrijving.Item1?.Regel2,
				//		Regel3 = verblijfplaatsGemeenteVanInschrijving.Item1?.Regel3,
				//		Straat = verblijfplaatsGemeenteVanInschrijving.Item1?.Straat,
				//		Woonplaats = verblijfplaatsGemeenteVanInschrijving.Item1?.Woonplaats
				//	})
				//	.ToList(),
				//BewoningPeriodes = (MapBewoningPeriodes(medebewoners)?.ToList())
			};
	...
	//private static IEnumerable<GbaBewoningPeriode>? MapBewoningPeriodes(IEnumerable<lo3_pl_persoon> medebewoners)
	//{
	//	if (medebewoners?.Any() == false)
	//	{
	//		return Enumerable.Empty<GbaBewoningPeriode>();
	//	}

	//	return new List<GbaBewoningPeriode>
	//	{
	//		new GbaBewoningPeriode
	//			{
	//				Bewoners =  medebewoners?.Count() < 20
	//					? medebewoners.Select(medebewoner => new GbaBewoner
	//					{
	//						Burgerservicenummer = medebewoner?.burger_service_nr?.ToString().PadLeft(9, '0'),
	//					}).ToList()
	//					: null,
	//				IndicatieVeelBewoners =  medebewoners?.Count() >= 20 ? true : null
	//			}
	//	};
	//}
}