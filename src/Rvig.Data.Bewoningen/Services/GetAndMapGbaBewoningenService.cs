...
public class GetAndMapGbaBewoningenService : GetAndMapGbaServiceBase, IGetAndMapGbaBewoningenService
{
	...
	private async Task<(IEnumerable<GbaBewoning> bewoningen, int afnemerCode)> GetMappedBewoningen(string identificatie, bool checkAuthorization, Func<string, Task<DbBewoningWrapper?>> getBewoningDataObjectFunc, Func<List<(bewoning_bewoner, long)>, List<(bewoning_bewoner, long)>, string?, DateTime?, DateTime?, DateTime?, GbaBewoning?> getMappedBewoningObjectFunc, DateTime? peildatum = null, DateTime? van = null, DateTime? tot = null)
	{
		...
		if (dbBewoningWrapper != null && !string.IsNullOrWhiteSpace(dbBewoningWrapper.verblijf_plaats_ident_code))
		{
			...
			int lastIndex = dates.FindIndex(date => date.Equals(dates.Last()));
			...
			foreach (var date in dates)
			{
				...
				if (index != lastIndex)
				{
					endDateTime = DateTime.ParseExact(dates.ElementAt(index + 1)!.Value.ToString()!, "yyyyMMdd", CultureInfo.InvariantCulture);
					...
					bewonerFilteringResultsWithPeriods.Add((bewonersPlIds: dbBewonersPlIds, mogelijkeBewonersPlIds: dbMogelijkeBewonersPlIds, startDateTime: startDateTime, endDateTime: endDateTime));
				}
			}
	...
	{
		var onvolledigeDatum = new DatumOnvolledig(onvolledigeDateInt?.ToString());

		if (onvolledigeDatum.IsCompleteOnvolledig())
		{
			return int.Parse(DateTime.MaxValue.ToString("yyyyMMdd"));
		}
		else if (!onvolledigeDatum.Dag.HasValue && onvolledigeDatum.Maand.HasValue)
		{
			onvolledigeDatum.Maand = isEndDate ? onvolledigeDatum.Maand + 1 : onvolledigeDatum.Maand;
			onvolledigeDatum.Dag = 1;
		}
		else if (!onvolledigeDatum.Dag.HasValue && !onvolledigeDatum.Maand.HasValue)
		{
			onvolledigeDatum.Jaar = isEndDate ? onvolledigeDatum.Jaar + 1 : onvolledigeDatum.Jaar;
			onvolledigeDatum.Maand = 1;
			onvolledigeDatum.Dag = 1;
		}

		return int.Parse($"{onvolledigeDatum.Jaar}{onvolledigeDatum.Maand.ToString()?.PadLeft(2, '0')}{onvolledigeDatum.Dag.ToString()?.PadLeft(2, '0')}");
	}
	...
	private (List<(bewoning_bewoner dbBewoner, long plId)> dbBewonersPlIds, List<(bewoning_bewoner dbBewoner, long plId)> dbMogelijkeBewonersPlIds) GetBewonersAndMogelijkeBewoners(List<(bewoning_bewoner dbBewoner, long plId)> bewonersPlIds, DateTime? peildatum, DateTime? van, DateTime? tot)
	{
		...
		(List<(bewoning_bewoner dbBewoner, long plId)> dbBewonersPlIds, List<(bewoning_bewoner dbBewoner, long plId)> dbMogelijkeBewonersPlIds) = (Enumerable.Empty<(bewoning_bewoner dbBewoner, long plId)>().ToList(), Enumerable.Empty<(bewoning_bewoner dbBewoner, long plId)>().ToList());
