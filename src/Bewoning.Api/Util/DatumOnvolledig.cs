...
public class DatumOnvolledig
{
    ...
    private void ParseToDatumOnvolledig(string? incompleteDateString)
    {
        if (!string.IsNullOrWhiteSpace(incompleteDateString) && incompleteDateString != "00000000" && incompleteDateString != "0" && incompleteDateString != "0000-00-00")
        {
            ...
            {
                jaar = incompleteDateString.Replace("-", "").Length >= 4 ? int.Parse(incompleteDateString.Substring(0, 4)) : null;
                maand = incompleteDateString.Replace("-", "").Length >= 6 ? int.Parse(incompleteDateString.Substring(5, 2)) : null;
                dag = incompleteDateString.Replace("-", "").Length == 8 ? int.Parse(incompleteDateString.Substring(8, 2)) : null;
            }
            else
            {
