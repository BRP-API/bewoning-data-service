...
namespace Rvig.Data.Base.Postgres.Repositories.Queries.Helper
{
...
where {0} and natnltt.volg_nr = 0 and onjuist_ind is null";
        ...
        public static (string where, NpgsqlParameter pgsqlParam) CreateBurgerservicenummerWhere(string bsn)
        {
            ...
            return ("where burger_service_nr = @BSN and persoon_type = 'P' and pers.stapel_nr = 0 and pers.volg_nr = 0 and ((pl.bijhouding_opschort_reden is not null and pl.bijhouding_opschort_reden != 'W' and pl.bijhouding_opschort_reden != 'F') or pl.bijhouding_opschort_reden is null)", new NpgsqlParameter("BSN", long.Parse(bsn)));
        }
        ...
        private static (string where, IEnumerable<NpgsqlParameter> parameters) CreateBsnPart(IEnumerable<string> burgerservicenummers)
        {
            ...
            if (burgerservicenummers.Count() == 1)
            {
                return ("burger_service_nr = @BSN",
                    new List<NpgsqlParameter> { new("BSN", long.Parse(burgerservicenummers.Single())) });
            }
            else
            {
                var bsnIndex = 0;
